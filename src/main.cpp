...
void generate_path_RRT() {
    ...
    new_tree = rrtTree(waypoints, map, map_origin_x, map_origin_y, res, margin);

    std::cout << "Generating Path" << std::endl;
    path_RRT = new_tree.generateRRT(world_x_max, world_x_min, world_y_max,
    ...
    if (path_RRT.size() != 0) {
        ...
        new_tree.visualizeTree(path_RRT);
    }
}
